...
<table class="tablesaw table-striped table-bordered tablesaw-columntoggle" data-tablesaw-mode="columntoggle" data-tablesaw-minimap="" id="table-3973">
		<thead>
		  <tr>
			  ...
			  <th data-tablesaw-priority="4">{{ trans('app.start_date')}}</th>
			   <th data-tablesaw-priority="4">{{ trans('app.end_date')}}</th>
		...
		@foreach($data as $view)
			<tr>
			  <td class="tablesaw-priority-5 tablesaw-cell-visible">{{$view->title}}</td>
             
			  <td class="tablesaw-priority-3">{{$view->category}}</td>
			  <td class="tablesaw-priority-3">{{$view->start_date}}</td>
			   <td class="tablesaw-priority-3">{{$view->end_date}}</td>
			  <td class="tablesaw-priority-1">		
			  <a title="{{ trans('app.edit')}}" data-toggle="tooltip" data-placement="top" data-original-title="{{ trans('app.edit')}}" class="btn btn-icon btn-info btn-outline btn-round" href="{{url('productedit/'.'/'.$view->id)}}"><i class="icon wb-pencil" aria-hidden="true"></i> </a> 
				
				<button data-placement="top" data-toggle="modal" rel="tooltip" title="{{ trans('app.delete')}}"  data-original-title="{{ trans('app.delete')}}"  class="btn btn-icon btn-danger btn-outline btn-round" data-target=".exampleNiftyFlipVertical"  type="button" data-href="{{URL::to('eventdestroy',$view->id)}}"><i class="icon fa-remove" aria-hidden="true"></i></button>
				</td>			
			</tr>
		@endforeach
